//Для классов Date и Time (задача 4.3.) реализуйте перегрузку операторов сравнения. Какие еще
//операторы Вы сочтете возможным и полезным перегрузить?

//Помимо операторов сравнения (==, !=, <, >, <=, >=), полезными могут быть операторы ввода и вывода (>> и <<),
//которые позволят удобно вводить и выводить даты и время.

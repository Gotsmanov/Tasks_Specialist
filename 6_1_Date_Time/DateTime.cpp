//Оператор << для вывода объектов класса Date и Time в потоке. В данном случае используется std::setwи std::setfillдля форматирования результата.
//Оператор >> для ввода объектов класса Date и Time потока. После регистрации результатов используется метод Set для установки результатов измерений полей объекта.
